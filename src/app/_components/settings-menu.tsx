  ...
  return (
    <div ref={ref} className="relative" data-wm-id="dashboard.settings-menu">
      ...
      {open && (
        <div
          role="menu"
          className="absolute right-0 top-full mt-1 w-72 rounded-xl border border-line bg-surface shadow-lg overflow-hidden z-50"
...
function SectionHeader({ label }: { label: string }) {
  return (
    <div className="px-3 pt-2 pb-1 text-[9px] font-mono uppercase tracking-[0.18em] text-faint">
      {label}
    </div>
  );
}
...
function MenuLink({
  ...
  const className =
    "flex items-start gap-3 px-3 py-2.5 hover:bg-surface-2 transition-colors";
  const body = (
    <>
